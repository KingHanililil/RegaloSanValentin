...
const innColor = new Color("#ff69b4");
const outColor = new Color("#8a2be2");
...
void main() {
    ...
    gl_Position = projectionMatrix * mvp;
    float adjustedSize = size * (1.0 + sin(vProgress * PI) * 1.5);
...
void main() {
    ...
    float fade = pow(sin(vProgress * PI), 2.0);
    gl_FragColor = vec4(vColor, a * fade * 0.9);
}
`,
    transparent: true,
    depthTest: false,
...
renderer.setAnimationLoop(() => {
  ...
  galaxy.rotation.y = elapsedTime * rotationSpeed;
  universe.rotation.y = elapsedTime * rotationSpeed / 3;
  spriteGroup.rotation.y = galaxy.rotation.y;
...
window.addEventListener('load', () => {
  ...
  // Intentar desmutar inmediatamente 
  setTimeout(() => {
    if (audio) {
      audio.muted = false;
